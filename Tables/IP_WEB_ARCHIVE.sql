...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[IP_WEB_ARCHIVE]') AND type in (N'U'))
BEGIN
	DROP TABLE [dbo].[IP_WEB_ARCHIVE]

	IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[IP_WEB_ARCHIVE]') AND type in (N'U'))
		PRINT '*** Unable to drop Table: [dbo].[IP_WEB_ARCHIVE]'
	ELSE
		PRINT 'DROPPED Table: [dbo].[IP_WEB_ARCHIVE]'
END 
GO

/****** Object:  Table [dbo].[IP_WEB_ARCHIVE]    Script Date: 13-nov-2013 ******/
...
CREATE TABLE [dbo].[IP_WEB_ARCHIVE](
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[IP_WEB_ARCHIVE]') AND type in (N'U'))
	PRINT 'Created Table: [dbo].[IP_WEB_ARCHIVE]'
ELSE
	PRINT '*** Unable to create Table: [dbo].[IP_WEB_ARCHIVE]'
GO
