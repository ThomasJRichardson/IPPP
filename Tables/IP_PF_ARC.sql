...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[IP_PF_ARC]') AND type in (N'U'))
BEGIN
	DROP TABLE [dbo].[IP_PF_ARC]

	IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[IP_PF_ARC]') AND type in (N'U'))
		PRINT '*** Unable to drop Table: [dbo].[IP_PF_ARC]'
	ELSE
		PRINT 'DROPPED Table: [dbo].[IP_PF_ARC]'
END 
GO

/****** Object:  Table [dbo].[IP_PF_ARC]    Script Date: 13-nov-2013 ******/
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[IP_PF_ARC]') AND type in (N'U'))
	PRINT 'Created Table: [dbo].[IP_PF_ARC]'
ELSE
	PRINT '*** Unable to create Table: [dbo].[IP_PF_ARC]'
GO
