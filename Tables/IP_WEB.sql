...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[IP_WEB]') AND type in (N'U'))
BEGIN
	DROP TABLE [dbo].[IP_WEB]

	IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[IP_WEB]') AND type in (N'U'))
		PRINT '*** Unable to drop Table: [dbo].[IP_WEB]'
	ELSE
		PRINT 'DROPPED Table: [dbo].[IP_WEB]'
END 
GO

/****** Object:  Table [dbo].[IP_WEB]    Script Date: 13-nov-2013 ******/
...
CREATE TABLE [dbo].[IP_WEB](
	[Username]		[varchar](15)	NOT NULL PRIMARY KEY,		--PROFUND Person.Reference. starts IP-
	[PersonUID]		[int]		NULL,				--PROFUND Person.PersonUID

	[PPSN]			[varchar](20)	NULL,		--either APT or Member
	...
	[DateEmpStart_1]	[datetime]	NOT NULL,	--only APT
	[DateEmpCease_1]	[datetime]	NULL,
	[DateJoinedScheme_1]	[datetime]	NULL,
	[SchemeCategory_1]	[varchar](100)	not null,			--PROFUND JobClass.LongDesc
	...
	[DateEmpStart_m1]	[datetime]	not NULL,	--only Member
...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[IP_WEB]') AND type in (N'U'))
	PRINT 'Created Table: [dbo].[IP_WEB]'
ELSE
	PRINT '*** Unable to create Table: [dbo].[IP_WEB]'
GO
